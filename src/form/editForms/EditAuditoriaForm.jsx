...
const EditAuditoriaForm = ({ show, onClose, fetchData, id_auditoria, name, audtoriasList}) => {
  ...
  const [auditoriaName, setAuditoriaName] = useState(name);
  const [showDeletePopup, setShowDeletePopup] = useState(false);

  useEffect(()=>{
    const trimmed = auditoriaName.trim();
    if (trimmed.length === 0) {
      setErrorMessage('El nombre debe tener al menos un carácter');
    }
    else if (
      audtoriasList.includes(trimmed) &&
      trimmed !== name  // si cambias el propio nombre viejo, no debería contar como duplicado
    ) {
      setErrorMessage('Ese nombre ya está en uso');
    }
    else {
      setErrorMessage('');
    }
  }, [auditoriaName, audtoriasList, name]);
  ...
  const confirmDelete = async () => {
    setLoading(true);
      ...
      try {
        const response = await fetch('https://4qznse98v1.execute-api.eu-west-1.amazonaws.com/dev/updateAuditoria', {
          method: 'DELETE',
          headers: {
    ...
    <div className="popup-overlay">
      ...
      <button className="popup-close" onClick={handleClose}>
        ...
        <form className="mx-auto" onSubmit={handleSubmit}>
          <h4 className="text-lg font-semibold mb-5">EDITAR AUDITORIA</h4>
          <div className="grid md:grid-cols-1 md:gap-6 w-1/2">
            <div className=''>
              <div className="">
                <Input
                  label="Nombre"
                  ...
                  name="name"
                  value={auditoriaName}
                  onChange={(e) => setAuditoriaName(e.target.value)}
                  required
          ...
          <Button
            ...
            disabled={loading || Boolean(errorMessage)}
          >
            {loading ? 'Cargando...' : 'Eliminar Auditoría'}
          </Button>
          </div>
        </form>
        {
      ...
      {showDeletePopup && (
        <DeleteForm
          ...
          message={`
            Al borrar esta auditoría se borrarán todas las evidencias
          ...
          loading={loading}
          bottomMessage={'¿Estás seguro que quieres eliminar la auditoría?'}
        />
      )}
    </div>
  );
...
export default EditAuditoriaForm;