  ...
  const resendPassword = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            'https://4qznse98v1.execute-api.eu-west-1.amazonaws.com/dev/resendPassword',
            requestBody,
            config
        );
        ...
        if (response.status === 200) {
          ...
          // por si acaso necesitas manejar otros 2xx
          setSuccessMessage('');
          setErrorMessage(response.data.error || 'Error desconocido');
        }
        if (response.status === 200) {
            setSuccessMessage("Contraseña temporal actualizada correctamente");
        } else {
  ...
  const changePassword = async () => {
    ...
    try {
        ...
        if (error.response && error.response.data) {
          ...
          // Si no es cadena, conviértela a cadena
          if (typeof errorMsg !== 'string') {
              errorMsg = JSON.stringify(errorMsg);
          }
          if (errorMsg.includes("Password must have symbol characters")) {
    ...
    <div className="popup-overlay">
      <div className="popup relative">
        ...
        <form className="max-w-xl mx-auto w-full">
          ...
          {(userData.is_responsable === 0 && userData.is_gestor === 0) || (userData.is_gestor === 1) &&
            <div className='flex justify-around w-full'>
              <Button
            ...
            <Button
              onClick={(e)=>{e.preventDefault(); resendPassword()}}
              className={`long${
                loading ? 'opacity-50 cursor-not-allowed' : ''
              }`}
              disabled={loading}
            >
              {loading ? 'Cargando...' : 'Reenviar Contraseña Temporal'}
            </Button>
            </div>
          }
        </form>
        {
          ...
          successMessage ? 
          (
          <p className='h7 text-center' style={{ color: 'green' }}>{successMessage}</p>
      ...
      {showChangePassword &&(
        ...
        <div className='relative ml-12 w-1/4 h-1/2 bg-white rounded-3xl'>
            {loading && (
              <div className="absolute top-0 left-0 w-full h-full bg-gray-400 bg-opacity-70 flex justify-center items-center z-10 rounded-3xl">
                <div role="status">
                  <svg aria-hidden="true" className="inline w-10 h-10 text-gray-200 animate-spin dark:text-gray-600 fill-yellow-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
