...
const ShowFile = ({ show, onClose, imgkey, bucketName, id_control, id_auditoria, state, fetchData, control_name, message_admin, order}) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log(`Archivo ${imgkey} eliminado exitosamente del bucket ${bucketName}`);
      // Refrescar datos o cerrar modal luego de la eliminación
      const response = await fetch('https://4qznse98v1.execute-api.eu-west-1.amazonaws.com/dev/deleteFile', {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        onClose();
        setLoading(false);
        //messagePopUp('Archivo eliminado correctamente', 'success')
      } else{
        //messagePopUp('Error eliminando el archivo', 'error')
      }
      onClose();
    } catch (error) {
  ...
  return (
    <div className="popup-overlay">
      ...
      {message_admin !== 'None' && role === 'responsable' && 
      <div 
      className='ml-6 px-3 pt-12 transition-opacity duration-300 ease-in-out opacity-100 flex-row'
      style={{
        height: '90%', 
        width: '30%', 
      ...
      {loading && (
        ...
        <label htmlFor="message" className="block mb-2 text-md font-bold text-black">Comentario</label>
        <div className='my-auto h-2/3 border-solid border-2 border-gray-400 rounded-3xl py-2 cursor-default'>
          <p className='text-justify'>{message_admin != 'None' ? (message_admin) : ''}</p>
        </div>
      </div>
      }
